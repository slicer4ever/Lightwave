...
struct LWSQuaternion<float> {
	...
	static LWSQuaternion FromEuler(float Pitch, float Yaw, float Roll) {
	...
	static LWSQuaternion FromEuler(const LWVector3<float> &Euler) {
		return FromEuler(Euler.x, Euler.y, Euler.z);
	}

	static LWSQuaternion FromAxis(float xAxis, float yAxis, float zAxis, float Theta) {
		float s = sinf(Theta*0.5f);
		return LWSQuaternion(cosf(Theta*0.5f), xAxis*s, yAxis*s, zAxis*s);
	}

	static LWSQuaternion FromAxis(const LWVector4<float> &AxisAngle) {
		return FromAxis(AxisAngle.x, AxisAngle.y, AxisAngle.z, AxisAngle.w);
	}

	static LWSQuaternion SLERP(const LWSQuaternion<float> &A, const LWSQuaternion<float> &B, float t) {
	...
	static LWSQuaternion NLERP(const LWSQuaternion<float> &A, const LWSQuaternion<float> &B, float t) {
	...
	LWSVector4<float> ToEuler(void) const {
		...
		__m128 YSq = _mm_mul_ps(Sq, _mm_set_ps(1.0f, -1.0f, -1.0f, 1.0f));
		__m128 RSq = _mm_mul_ps(Sq, _mm_set_ps(1.0f, -1.0f, 1.0f, -1.0f));
	...
	LWSQuaternion Conjugate(void) const {
		return _mm_mul_ps(m_Data, _mm_set_ps(-1.0f, -1.0f, -1.0f, 1.0f));
	}

	LWSQuaternion Inverse(void) const {
		__m128 iLenSq = _mm_div_ps(_mm_set_ps1(1.0f), _mm_dp_ps(m_Data, m_Data, 0xFF));
		return _mm_mul_ps(_mm_mul_ps(m_Data, _mm_set_ps(-1.0f, -1.0f, -1.0f, 1.0f)), iLenSq);
	}
	...
	LWSQuaternion operator+(const LWSQuaternion<float> &Rhs) const {
	...
	LWSQuaternion operator*(const LWSQuaternion<float> &Rhs) const {
		...
		mxyzx = _mm_mul_ps(mxyzx, _mm_set_ps(1.0f, 1.0f, 1.0f, -1.0f));
		myzxy = _mm_mul_ps(myzxy, _mm_set_ps(1.0f, 1.0f, 1.0f, -1.0f));
		mzxyz = _mm_mul_ps(mzxyz, _mm_set_ps1(-1.0f));
	...
	LWSQuaternion operator*(float Rhs) const {
	...
	LWSQuaternion &operator*=(const LWSQuaternion<float> &Rhs) {
		*this = (*this*Rhs);
		return *this;
	}

	LWSQuaternion &operator*=(float Rhs) {
	...
	LWSQuaternion operator-() const {
		return _mm_mul_ps(m_Data, _mm_set_ps1(-1.0f));
	}
	...
	LWSQuaternion(__m128 Data) : m_Data(Data) {}

	LWSQuaternion(float vw, float vx, float vy, float vz) : m_Data(_mm_set_ps(vw, vz, vy, vx)) {}
	/*
	LWSQuaternion(const LWSMatrix4<float> &Mat) {
		
		LWVector4<float> R0 = Mat.m_Rows[0].AsVec4();
		LWVector4<float> R1 = Mat.m_Rows[1].AsVec4();
		LWVector4<float> R2 = Mat.m_Rows[2].AsVec4();
		LWVector4<float> R3 = Mat.m_Rows[3].AsVec4();
	...
	LWSQuaternion() : m_Data(_mm_set_ps(1.0f, 0.0f, 0.0f, 0.0f)) {}
};

>>>>>>> Added quaternion simd variant.
#endif