...
struct LWSVector4<double> {
	...
	friend LWSVector4<double> operator - (const LWSVector4<double> &Rhs) {
		__m256d t = _mm256_set1_pd(-1.0f);
		return _mm256_mul_pd(Rhs.m_Data, t);
	}
	...
	LWSVector4(const LWVector4<double> &vxyzw) : m_Data(_mm256_set_pd(vxyzw.w, vxyzw.z, vxyzw.y, vxyzw.x)) {}

	LWSVector4(const LWVector3<double> &vxyz, double vw) : m_Data(_mm256_set_pd(vw, vxyz.z, vxyz.y, vxyz.x)) {}

	LWSVector4(const LWVector2<double> &vxy, const LWVector2<double> &vzw) : m_Data(_mm256_set_pd(vzw.y, vzw.x, vxy.y, vxy.x)) {}

	LWSVector4(const LWVector2<double> &vxy, double vz, double vw) : m_Data(_mm256_set_pd(vw, vz, vxy.y, vxy.x)) {}

	LWSVector4(double vx, double vy, double vz, double vw) : m_Data(_mm256_set_pd(vw, vz, vy, vx)) {}

	LWSVector4(double f = 0.0) : m_Data(_mm256_set_pd(f, f, f, f)) {}
};
