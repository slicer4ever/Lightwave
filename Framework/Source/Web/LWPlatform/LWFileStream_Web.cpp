...
bool LWFileStream::GetFolderPath(uint32_t FolderID, char *Buffer, uint32_t BufferLen) {
	...
	if (FolderID == Fonts) snprintf(Buffer, BufferLen, ""); //Default assumed font path.
	else if (FolderID == User) snprintf(Buffer, BufferLen, ""); //Default assumed user path.
	else if (FolderID == Game) snprintf(Buffer, BufferLen, ""); //Default assumed game storage path.
