...
uint32_t LWFileStream::GetFolderPath(uint32_t FolderID, char8_t *Buffer, uint32_t BufferLen) {
	...
	if (FolderID == Fonts) return snprintf((char*)Buffer, BufferLen, ""); //Default assumed font path.
	else if (FolderID == User) return snprintf((char*)Buffer, BufferLen, ""); //Default assumed user path.
	else if (FolderID == Game) return snprintf((char*)Buffer, BufferLen, ""); //Default assumed game storage path.
